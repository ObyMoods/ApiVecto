...
class ToolsController {
  ...
  if (!key) {
    return { valid: false, message: "Key not provided" };
  }

  if (!fs.existsSync(KEY_FILE)) {
    return { valid: false, message: "Key database not found" };
  }

  const keyList = JSON.parse(fs.readFileSync(KEY_FILE));

  const keyData = keyList.find(k => k.sessionKey === key);

  if (!keyData) {
    return { valid: false, message: "Invalid key" };
  }

  return { valid: true, keyInfo: keyData };
}
